...
import LinkEditForm from '../components/linkform'
...
class LinkList extends Component {
    ...
    request = () => {//status:状态码，pages:查询页数
        ...
        http.post('/link/list', params).then(res => {
            console.log(res);
            if (res === undefined) {
    ...
    changeLinkStatus = (id, status) => {
        ...
        http.post('/link/modifyState', params).then(res => {
            console.log(res);
            if (res === undefined) {
                message.error('网络问题,或服务器异常，稍后重试');
                return;
            ...
            if (res.status === 200) {
                ...
                _this.setState({
                    ...
                    message.success('修改链接状态成功');
                })
            } else {
                message.error('修改链接状态失败');
            }
        });
    }
    ...
    settingSorting = (id, sortnum) => {
        ...
        http.post('/link/setSorting', params).then(res => {
            console.log(res);
            if (res.status === 200) {
                _this.request();
    ...
    //提交修改链接
    updateLinkSubmit = (params) => {
        let _this = this;
        params.id = _this.state.linkID;
        http.post('/link/update', params).then(res => {
            console.log(res);
            ...
            <div className='article-list'>
                        ...
                        <ul className='dataSource'>
                            {
                                this.state.dataSource.map((item, index) => {
                                    return <li className='item' key={index}>
                                        ...
                                        <span className='data'>
                                            <img className='link-logo' src={item.logo}></img>
                                        </span>
                                        <span className='data'>{item.link}</span>
                                        <span className='data'>{item.createUserInfo.nickname}</span>
                                        ...
                                        <span className='data'>
                                            <Switch
                                                ...
                                                onClick={() => this.changeLinkStatus(item.id, item.isDel)}
                                            />
                                        </span>
                                        <span className='data'>
                                            <Button
                                            ...
                                            <Button
                                                size='small'
                                                type='primary'
                                                onClick={() => this.changeLinkButton(item)}
                                            >修改</Button>
                                        </span>
                                    </li>
                                })
                ...
                >
                    ...
                    <LinkEditForm
                        ...
                        formDataSubmit={this.updateLinkSubmit}
                    ></LinkEditForm>
                </Modal>
